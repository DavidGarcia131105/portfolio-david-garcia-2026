    ...
    <section id="about" style={{ padding: isMobile ? '48px 24px' : '64px 48px', maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr auto', gap: isMobile ? 24 : 48, alignItems: 'center' }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: '#666', lineHeight: 1.85, margin: '0 0 12px' }} dangerouslySetInnerHTML={{ __html: t('about.p1') }} />
          <p style={{ fontSize: 13, color: '#666', lineHeight: 1.85, margin: 0 }}>{t('about.p2')}</p>
          <div style={{ display: 'flex', gap: 8, marginTop: 18, flexWrap: 'wrap' }}>
            {chips.map(({ key, color }) => (
              <span key={key} style={{ background: '#1a1a1a', border: '1px solid #222', borderRadius: 6, padding: '5px 12px', fontSize: 11, color: '#aaa', display: 'flex', alignItems: 'center', gap: 6 }}>
